...
export class GoogleSheetApiService {
  ...
  public get<T>(spreadsheetId: string, worksheetName: string, attributesMapping: object | string[]): Observable<T[]> {
    return this.getRows(spreadsheetId, worksheetName)
               .pipe(
                 map((rows: string[][]) => 
                  this.rowsToEntries(rows).map((entry: object) => 
  ...
  public rowsToEntries(rows: string[][]): object[] {
    ...
    return rows.slice(1).map((row: Array<string>) =>
      columns.reduce((entry: any, columnName: string, idx: number) => {
  ...
  private getValueFromEntry(entry: any, attribute: string): string | null {
  ...
  private getListFromEntry(entry: object, attribute: string): string[] {
    ...
    let i = 1;
    let curElement: string | null = this.getValueFromEntry(entry, `${attribute}${i}`);
    while (curElement) {
